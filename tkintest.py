...
root = Tk()
root.title('Dessin turle par Tkinter')
...
class Movements:
    ...
    def exit(self):
        q = messagebox.askyesno('Exit.', 'voulez-vous quitter ??'.title())
        if q == True:
            root.destroy()
        else:
            pass
...
forward_btn = ttk.Button(root, text='En avant',
                         style='TButton', command=pro.forward)
backward_btn = ttk.Button(root, text='En arriére',
                          style='TButton', command=pro.backward)
right_btn = ttk.Button(root, text='à droite', style='TButton', command=pro.right)
left_btn = ttk.Button(root, text='à gauche', style='TButton', command=pro.left)
...
cbox.set('Change la couleur !')
cbox.grid(row=1, column=5)
cbox['state'] = 'readonly'

ok_btn = ttk.Button(root, text='Enregistrer', style='TButton',
...
bbox.set('la couleur de l\'arriére plan!')
bbox['state'] = 'readonly'

ok2_btn = ttk.Button(root, text='Enregistrer', style='TButton',
                     command=pro.board_color)
ok2_btn.grid(row=3, column=4)


btn1 = ttk.Button(root, text='Tortue', style='TButton',
                  command=lambda: pro.shape('turtle'))
btn2 = ttk.Button(root, text='Carrée', style='TButton',
                  command=lambda: pro.shape('square'))
btn3 = ttk.Button(root, text='Cercle', style='TButton',
...
exit_btn = Button(root, text='Quitter', command=pro.exit)
